...
public class CannyEdgeDetection {
    ...
    public static float[][] getGradients(int[][][] input, int width, int height) {
        float[][] gradients = new float[width][height];
        float[][] brightnessMap = new float[width][height];
        int[] zero = {0, 0, 0};
        for (int i = 0; i < width; i++) {
        	for (int j = 0; j < height; j++) {
        		int[] color = {input[0][i][j], input[1][i][j], input[2][i][j]};
        ...
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                ...
                if (j - 1 >= 0) {
                    ...
                    gradients[i][j] += -0.5 * b;
					*/
                	gradients[i][j] += -0.5 * brightnessMap[i][j - 1];
                }
                ...
                if (i + 1 < width) {
                    ...
                    gradients[i][j] += 0.5 * b;
                    */
                	gradients[i][j] += 0.5 * brightnessMap[i + 1][j];
                }
